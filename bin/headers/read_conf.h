...
static void read_custom_binary_gauge_field_andreas(double **gauge, char *fname, QudaGaugeParam *param, QudaInvertParam *inv_param, int gridSize[4])
{
  ...
  starts[0] = 0;
  starts[1] = comm_coords(0)*param->X[0];
  starts[2] = comm_coords(1)*param->X[1];
  starts[3] = comm_coords(2)*param->X[2];
  starts[4] = comm_coords(3)*param->X[3];
  starts[5] = 0;
	...
	for(int iy = 0; iy < param->X[1]; iy++){
	  for(int iz = 0; iz < param->X[2]; iz++){
		...
		{  
		  ...
		  p_gauge[mu][pos*3*3+ic1*3+ic2] = *((Complex*) &(ftmp[i]));
		  i += sizeof(Complex);
		} 
	    } 
	  } 
