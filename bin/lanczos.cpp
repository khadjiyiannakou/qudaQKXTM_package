...
extern int arnoldi_P;
extern int arnoldi_K;
...
int main(int argc, char **argv)
{
  ...
  if (dslash_type == QUDA_TWISTED_MASS_DSLASH) {
    ...
    inv_param.twist_flavor = QUDA_TWIST_PLUS; // change later
  }
  ...
  lInfo.dimensionMatrix = gauge_param.X[0] * gauge_param.X[1] * gauge_param.X[2] * gauge_param.X[3] * 4 * 3;
  lInfo.lanczosTotalVectors = arnoldi_K + arnoldi_P;
  lInfo.lanczosWantedVectors = arnoldi_K;
  lInfo.lanczosUnwantedVectors = arnoldi_P;
  lInfo.maxLanczosIter=100000;
  lInfo.tolerance=1e-8;
  lInfo.seed = 0;
  lInfo.smlm = SM;

  for (int dir = 0; dir < 4; dir++) {
    gauge[dir] = malloc(V*gaugeSiteSize*gSize);
    gaugeAPE[dir] = malloc(V*gaugeSiteSize*gSize);
    if(gauge[dir] == NULL){
  ...
  if (strcmp(latfile,"")) {  // load in the command line supplied gauge field
    readGaugeAndreas(gaugeAPE, latfile, &gauge_param, &inv_param, gridsize_from_cmdline);        // first read gauge field without apply BC
    for(int dir = 0 ; dir < 4 ; dir++) memcpy( gauge[dir] , gaugeAPE[dir] , V*gaugeSiteSize*gSize);

    mapNormalToEvenOddGauge(gauge,gauge_param,xdim,ydim,zdim,tdim);
  ...
  */
  ...
  if (strcmp(latfile,"")) {  // load in the command line supplied gauge field
    ...
    for(int dir = 0 ; dir < 4 ; dir++) memcpy( gaugeAPE[dir] , gauge[dir] , V*gaugeSiteSize*gSize);

    mapEvenOddToNormalGauge(gaugeAPE,gauge_param,xdim,ydim,zdim,tdim);
  ...
  double plaq = qkxTM_gaugeAPE->calculatePlaq();
  printfQuda("Plaquette is %e\n",plaq);
  delete qkxTM_gaugeAPE;
  /////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////

  IRLM_ES(gauge,&inv_param,&gauge_param,info,lInfo);
  /////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
