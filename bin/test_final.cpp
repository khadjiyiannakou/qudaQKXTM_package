...
int main(int argc, char **argv)
{
  ...
  if (strcmp(latfile,"")) {  // load in the command line supplied gauge field
    ...
    applyBoundaryCondition(gauge, V/2 ,&gauge_param);        // !!!!!! perform test                                          // apply BC on the field that we will pass to inverter
  ...
  char filename_out_proton[257] = "/home/khadjiyiannakou/qudaQKXTM_package/bin/test/corr_proton_final";
  char filename_out_neutron[257] = "/home/khadjiyiannakou/qudaQKXTM_package/bin/test/corr_neutron_final";
  FILE *ptr_out_proton = NULL, *ptr_out_neutron = NULL;
  ...
  for(int it = 0 ; it < tdim ; it++)
	...
	fprintf(ptr_out_proton,"%d %d %d %+e %+e\n",comm_coords(3)*tdim+it,gamma,gamma1,corr_fourier[it*4*4*2 + gamma*4*2 + gamma1*2 + 0],corr_fourier[it*4*4*2 + gamma*4*2 + gamma1*2 + 1]);
  }
