...
int nsmearHYP = 5;
double omega1HYP = 0.75;
double omega2HYP = 0.6;
...
void usage(char** argv )
{
  ...
  printf("    --load-gauge-smeared file                 # Load the smeared gauge file if you don't want to perform smearing\n");
  ...
  printf("    --t_sink                                  # Fix sink Position (default 0)\n");
  ...
  printf("    --Nstoch                                  # Number of stochastic noise vectors for stochastic three point function (default 1)\n");
  printf("    --nsmearHYP                               # Number of HYP smearing iterations (default 5)\n");
  printf("    --omega1HYP                               # omega1 parameter as it appears in HYP smearing\n");
  printf("    --omega2HYP                               # omega2 parameter as it appears in HYP smearing\n");
  printf("    --arnoldi_K                               # Number of eigenValues you want using Arnoldi (default 2)\n");
  printf("    --arnoldi_P                               # Number of expansion vectors using implicitly restarted Arnoldi (default 3)\n");

  printf("    --momP_fixed                              # Momentum to put on the sink for PDFs project\n");
...
int process_command_line_option(int argc, char** argv, int* idx)
{
  ...
  if( strcmp(argv[i], "--t_sink") ==0){
    if(i+1 >= argc){
      usage(argv);
    }
    t_sink = atoi(argv[i+1]);
    i++;
    ret = 0;
    goto out;
  }
  ...
  if( strcmp(argv[i], "--momP_fixed") ==0){
    if(i+1 >= argc){
      usage(argv);
    }
    momP_fixed = atoi(argv[i+1]);
    i++;
    ret = 0;
    goto out;
  }
  ...
  if( strcmp(argv[i], "--nsmearHYP") ==0){
    if(i+1 >= argc){
      usage(argv);
    }
    nsmearHYP = atoi(argv[i+1]);
    i++;
    ret = 0;
    goto out;
  }

    if( strcmp(argv[i], "--omega1HYP") ==0){
    if(i+1 >= argc){
      usage(argv);
    }
    omega1HYP = atof(argv[i+1]);
    i++;
    ret = 0;
    goto out;
  }
  ...
  if( strcmp(argv[i], "--omega2HYP") ==0){
    if(i+1 >= argc){
      usage(argv);
    }
    omega2HYP = atof(argv[i+1]);
    i++;
    ret = 0;
    goto out;
  }

  if( strcmp(argv[i], "--arnoldi_K") ==0){
    if(i+1 >= argc){
      usage(argv);
    }
    arnoldi_K = atoi(argv[i+1]);
    i++;
    ret = 0;
    goto out;
  }

  if( strcmp(argv[i], "--arnoldi_P") ==0){
    if(i+1 >= argc){
      usage(argv);
    }
    arnoldi_P = atoi(argv[i+1]);
    i++;
    ret = 0;
    goto out;
  }
