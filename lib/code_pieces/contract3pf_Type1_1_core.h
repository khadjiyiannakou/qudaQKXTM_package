 ...
 case 1:
   ...
   for(int mu = 0 ; mu < 4 ; mu++)
	 ...
	 for(int c2 = 0 ; c2 < 3 ; c2++){
	   prop1[mu][nu][c1][c2].x=1.;prop1[mu][nu][c1][c2].y=1.;
	 }
   */
 ...
 case 1:
   ...
   for(int mu = 0 ; mu < 4 ; mu++)
     for(int nu = 0 ; nu < 4 ; nu++)
       for(int c1 = 0 ; c1 < 3 ; c1++)
	 for(int c2 = 0 ; c2 < 3 ; c2++){
	   prop2[mu][nu][c1][c2].x=1.;prop2[mu][nu][c1][c2].y=1.;
	 }
   */
   ...
   for(int mu = 0 ; mu < 4 ; mu++)
     for(int nu = 0 ; nu < 4 ; nu++)
       for(int c1 = 0 ; c1 < 3 ; c1++)
	 for(int c2 = 0 ; c2 < 3 ; c2++)
	   prop2[mu][nu][c1][c2] = fetch_double2(uprop3DStochTex,sid3D + ( (mu*4+nu)*3*3 + c1*3 + c2 ) * space_stride);
   ...
   */
   ...
   for(int mu = 0 ; mu < 4 ; mu++)
     for(int nu = 0 ; nu < 4 ; nu++)
       for(int c1 = 0 ; c1 < 3 ; c1++)
	 for(int c2 = 0 ; c2 < 3 ; c2++)
	   prop2[mu][nu][c1][c2] = fetch_double2(dprop3DStochTex,sid3D + ( (mu*4+nu)*3*3 + c1*3 + c2 ) * space_stride);
...
for(int mu = 0 ; mu < 4 ; mu++)
  for(int c1 = 0 ; c1 < 3 ; c1++){
    insLine[mu][c1].x=1.;insLine[mu][c1].y=1.;
  }
*/
...
for(int idx = 0 ; idx < counter ; idx++){
  ...
  unsigned short int alpha1 = Cg5Cg5bar_ind[idx][3];
	  ...
	  for(int gamma = 0 ; gamma < 4 ; gamma++)
		...
		prop2[beta][beta1][b][b1] * xi[gamma][c] * prop1[alpha][alpha1][a][a1] * insLine[gamma1][c1];
	      
	      reg_out[gamma][gamma1] = reg_out[gamma][gamma1] + value *
		prop2[beta][beta1][b][b1] * xi[alpha][a] * prop1[gamma][gamma1][c][c1] * insLine[alpha1][a1];
	      
	      reg_out[gamma][gamma1] = reg_out[gamma][gamma1] - value *
		prop2[beta][beta1][b][b1] * xi[alpha][a] * prop1[gamma][alpha1][c][a1] * insLine[gamma1][c1];
