...
for(int mu = 0 ; mu < c_nSpin ; mu++)
  for(int ic = 0 ; ic < c_nColor ; ic++){
    ...
    out[mu*c_nColor*c_stride + ic*c_stride + latt_coord + evenSiteBit] = inEven[mu*c_nColor*half_stride + ic*half_stride + sid];
    out[mu*c_nColor*c_stride + ic*c_stride + latt_coord + oddSiteBit] = inOdd[mu*c_nColor*half_stride + ic*half_stride + sid];
