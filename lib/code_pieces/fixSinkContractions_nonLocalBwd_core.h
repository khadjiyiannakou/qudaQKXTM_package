...
switch(direction){
 case 0:
   pointMinus = LEXIC(t_id,z_id,y_id,(x_id-dl+c_localL[0])%c_localL[0],c_localL);
   break;
 case 1:
   pointMinus = LEXIC(t_id,z_id,(y_id-dl+c_localL[1])%c_localL[1],x_id,c_localL);
   break;
 case 2:
   pointMinus = LEXIC(t_id,(z_id-dl+c_localL[2])%c_localL[2],y_id,x_id,c_localL);
   break;
 }
...
for(int mu = 0 ; mu < 4 ; mu++)
  for(int nu = 0 ; nu < 4 ; nu++)
	...
	prop[mu][nu][c1][c2] = fetch_double2(fwdPropagatorTex,pointMinus + ( (mu*4+nu)*3*3 + c1*3 + c2 ) * c_stride);
      }
