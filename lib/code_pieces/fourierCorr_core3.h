...
for(int gamma = 0 ; gamma < 4 ; gamma++)
  for(int c1 = 0 ; c1 < 3 ; c1++)
    insLine[gamma*3+c1] = fetch_double2(insLineFourierTex ,  gamma*3*c_stride + c1*c_stride + it*c_localL[0]*c_localL[1]*c_localL[2] + sid  );

for(int gamma = 0 ; gamma < 4 ; gamma++)
  for(int c1 = 0 ; c1 < 3 ; c1++){

    shared_cache[cacheIndex] = insLine[gamma*3+c1] * expon;
    __syncthreads();
