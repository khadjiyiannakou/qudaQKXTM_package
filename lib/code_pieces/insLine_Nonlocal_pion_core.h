...
for(int pi = 0 ; pi < 4 ; pi++)
  for(int d = 0 ; d < 3 ; d++)
    Phi[pi][d] = fetch_double2(phiVectorStochTex,pointPlus + (pi*3+d)*c_stride);
  ...
  for(int phi = 0 ; phi < 4 ; phi++){
	...
	for(int gammap = 0 ; gammap < 4 ; gammap++)
	  for(int cp = 0 ; cp < 3 ;cp++){
	    for(int c1 = 0 ; c1 < 3 ; c1++)
	      for(int c2 = 0 ; c2 < 3 ; c2++)
		outIns[gammap][cp] = outIns[gammap][cp] + Phi[pi][c1] * g5xg[phi][pi] * conj(WilsonLink[c2][c1]) * conj(prop[phi][gammap][c2][cp]);
	  }
