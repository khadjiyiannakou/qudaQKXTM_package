...
double2 g5OnePgamma[4][4];
double2 g5OneMgamma[4][4];

for(int mu = 0 ; mu < c_nSpin ; mu++)
  for(int nu = 0 ; nu < c_nSpin ; nu++){
    g5OnePgamma[mu][nu].x = 0 ; g5OnePgamma[mu][nu].y = 0;
    g5OneMgamma[mu][nu].x = 0 ; g5OneMgamma[mu][nu].y = 0;
  }
...
switch( dir ){
 ...
 case 0: // g1
   g5OnePgamma[0][1].y = -1.; g5OnePgamma[0][2].x = 1.;
   g5OnePgamma[1][0].y = -1.; g5OnePgamma[1][3].x = 1.;
   g5OnePgamma[2][0].x = 1.; g5OnePgamma[2][3].y = 1.;
   g5OnePgamma[3][1].x = 1.; g5OnePgamma[3][2].y = 1.;

   g5OneMgamma[0][1].y = 1.; g5OneMgamma[0][2].x = 1.;
   g5OneMgamma[1][0].y = 1.; g5OneMgamma[1][3].x = 1.;
   g5OneMgamma[2][0].x = 1.; g5OneMgamma[2][3].y = -1.;
   g5OneMgamma[3][1].x = 1.; g5OneMgamma[3][2].y = -1.;

   break;
 case 1: // g2
   g5OnePgamma[0][1].x = -1.; g5OnePgamma[0][2].x = 1.;
   g5OnePgamma[1][0].x = 1.; g5OnePgamma[1][3].x = 1.;
   g5OnePgamma[2][0].x = 1.; g5OnePgamma[2][3].x = 1.;
   g5OnePgamma[3][1].x = 1.; g5OnePgamma[3][2].x = -1.;

   g5OneMgamma[0][1].x = 1.; g5OneMgamma[0][2].x = 1.;
   g5OneMgamma[1][0].x = -1.; g5OneMgamma[1][3].x = 1.;
   g5OneMgamma[2][0].x = 1.; g5OneMgamma[2][3].x = -1.;
   g5OneMgamma[3][1].x = 1.; g5OneMgamma[3][2].x = 1.;

   break;
 case 2: // g3
   g5OnePgamma[0][0].y = -1.; g5OnePgamma[0][2].x = 1.;
   g5OnePgamma[1][1].y = 1.; g5OnePgamma[1][3].x = 1.;
   g5OnePgamma[2][0].x = 1.; g5OnePgamma[2][2].y = 1.;
   g5OnePgamma[3][1].x = 1.; g5OnePgamma[3][2].y = -1.;

   g5OneMgamma[0][0].y = 1.; g5OneMgamma[0][2].x = 1.;
   g5OneMgamma[1][1].y = -1.; g5OneMgamma[1][3].x = 1.;
   g5OneMgamma[2][0].x = 1.; g5OneMgamma[2][2].y = -1.;
   g5OneMgamma[3][1].x = 1.; g5OneMgamma[3][2].y = 1.;
  ...
  for(int e = 0 ; e < 3 ; e++)
	...
	for(int phi = 0 ; phi < 4 ; phi++)
	  for(int gammap = 0 ; gammap < 4 ; gammap++){
	    if(norm(g5OneMgamma[pi][phi]) > 1e-4) outIns[gammap][cp] = outIns[gammap][cp] - conj(Phi[pi][d]) * g5OneMgamma[pi][phi] * gauge[d][e] * prop[phi][gammap][e][cp];
	  }
...
for(int d = 0 ; d < 3 ; d++)
  for(int e = 0 ; e < 3 ; e++)
	...
	for(int phi = 0 ; phi < 4 ; phi++)
	  for(int gammap = 0 ; gammap < 4 ; gammap++)
	    if(norm(g5OnePgamma[pi][phi]) > 1e-4) outIns[gammap][cp] = outIns[gammap][cp] + conj(Phi[pi][d]) * g5OnePgamma[pi][phi] * conj(gauge[e][d]) * prop[phi][gammap][e][cp];
