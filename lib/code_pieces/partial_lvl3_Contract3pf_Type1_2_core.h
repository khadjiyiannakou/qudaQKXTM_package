...
for(int beta1 = 0 ; beta1 < 4 ; beta1++)
  for(int b1 = 0 ; b1 < 3 ; b1++){
    reg_out[beta1][b1].x = 0.;     reg_out[beta1][b1].y = 0.; 
  }
...
for(int cc1 = 0 ; cc1 < 6 ; cc1++){
  ...
  for(int cc2 = 0 ; cc2 < 6 ; cc2++){
	...
	//	reg_out[alpha1][a1] = reg_out[alpha1][a1] + value *
	// prop2[beta][beta1][b][b1] * xi[alpha][a] * prop1[gamma][gamma1][c][c1];
	
	//reg_out[gamma][c1] = reg_out[gamma][c1] - value *
	// prop2[beta][beta1][b][b1] * xi[alpha][a] * prop1[gamma][alpha1][c][a1];
		
	//reg_out[alpha1][a1] = reg_out[alpha1][a1] - value *
	// prop2[beta][beta1][b][b1] * xi[gamma][c] * prop1[alpha][gamma1][a][c1];
	
	reg_out[beta1][b1] = reg_out[beta1][b1] + value *
	  prop1[gamma][gamma1][c][c1] * xi[beta][b] * prop1[alpha][alpha1][a][a1];

	reg_out[beta1][b1] = reg_out[beta1][b1] - value *
	  prop1[alpha][gamma1][a][c1] * xi[beta][b] * prop1[gamma][alpha1][c][a1];
...
for(int beta1 = 0 ; beta1 < 4 ; beta1++)
  for(int b1 = 0 ; b1 < 3 ; b1++)
    out[beta1*3*space_stride + b1*space_stride + sid] = reg_out[beta1][b1];
