...
for(int cc1 = 0 ; cc1 < 6 ; cc1++){
  ...
  for(int cc2 = 0 ; cc2 < 6 ; cc2++){
	...
	nu = Cg5Cg5bar_ind[idx][1];
	ku = Cg5Cg5bar_ind[idx][2];
	lu = Cg5Cg5bar_ind[idx][3];
	if(lu == c_nu){
	  for(int a = 0 ; a < 4 ; a++)
	    for(int b = 0 ; b < 4 ; b++)
		...
		spinor[nu][c3] = spinor[nu][c3] + factor * prop1[mu][b][c1][c1p] * prop1[a][ku][c2][c2p];
		spinor[nu][c3] = spinor[nu][c3] + factor * prop1[mu][ku][c1][c1p] * prop1[a][b][c2][c2p];
